...
export const BentoGridItem = ({
  ...
  const rightLists = ["JavaScript", "React Native", "Expo"];
  ...
  const handleCopy = () => {
    const text = "olaitandamilare230@gmail.com";
  ...
  return (
    <div
      className={cn("row-span-1 relative text-white-200 overflow-hidden rounded-3xl border border-white/[0.1] group/bento hover:shadow-xl transition duration-200 shadow-input dark:shadow-none justify-between flex flex-col space-y-4",className)}
      ...
      <div className={`${id === 6 && "flex justify-center"} h-full`}>
        ...
        <div
          ...
          <div className="font-sans font-extralight md:max-w-36 text-nowrap md:text-xs lg:text-base text-sm text-[#C1C2D3] z-10">
            {description}
          </div>
            ...
            <div className="flex gap-1 lg:gap-3 w-fit absolute -right-3 lg:-right-2">
              <div className="flex flex-col gap-3 md:gap-3 lg:gap-5">
                {leftLists.map((item, i) => (
                  <span key={i} className="py-2 px-3 text-xs lg:text-base opacity-50 lg:opacity-100 rounded-lg text-center bg-[#10132E]" >
                    {item}
                  </span>
                ))}
          ...
          {id === 6 && (
            <div className="mt-5 relative">
              ...
              <MagicButton
                ...
                otherClasses="!bg-[#161A31] text-nowrap"
              />
            </div>
          )}
        </div>
